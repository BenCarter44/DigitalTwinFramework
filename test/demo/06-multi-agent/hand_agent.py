...
from handwriting.hand_investigator import HandwritingInvestigator
...
class HandAgent(SciAgent):
    def __init__(self, flow: WorkflowEngine):
        super().__init__(flow)
        self.flow = flow

        # no learning. Simple investigator
        self.investigator = HandwritingInvestigator(flow)

        f = open("hand-agent-inference.out", "w")
        f.write("MODEL SELECTOR ========================= \n")
        f.close()

        f = open("hand-agent-learner.out", "w")
        ...
        async def model_select(
            ...
            f = open("hand-agent-inference.out", "a")
            f.write(
    ...
    async def main_loop(self, runtime: RuntimeAPI):
        ...
        f = open("hand-agent-learner.out", "a")
        f.write(
